const details = [
  {
    icon: "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/car.svg",
    label: "Vehicle",
  },
  {
    icon: "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/pasangers.svg",
    label: "0 Passengers",
  },
  {
    icon: "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/miles.svg",
    label: "0km / 0miles",
  },
  {
    icon: "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/clarity_clock-line.svg",
    label: "0hr 00min",
  },
];
const features = [
  "+500000 passengers transported",
  "Instant confirmation",
  "All-inclusive pricing",
  "Secure Payment by Stripe",
];

const paymentLogos = [
  "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/Paypal.png",
  "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/stripe-icon.png",
  "https://bookingform.britwayairporttransfer.co.uk/assets/front/images/cash-payment.png",
];
const airports = [
  "Southampton Airport, Southampton, SO18 2NL",

  "Manchester Airport, Manchester. M90 1QX",

  "Liverpool John Lennon Airport, Speke Hall Avenue, Speke, Liverpool, L24 1YD",

  "Leeds Bradford Airport, Whitehouse Lane, Yeadon, Leeds, LS19 7TU",

  "Edinburgh Airport, Edinburgh, EH12 9DN",

  "Coventry Airport, Rowley Road, Coventry, CV3 4FR",

  "Bristol Airport, Bristol, BS48 3DY",

  "Bournemouth Airport, Parley Lane, Christchurch, BH23 6SE",

  "Stansted Airport, Stansted, Essex. CM24 1RW",

  "Southend Airport, Southend-on-Sea, Essex. SS2 6YF",

  "Oxford, Kidlington Airport, Langford Lane, Kidlington, OX5 1RA",

  "Luton Airport, Luton, Bedfordshire. LU2 9LY",

  "London City Airport, London. E16 2PX",

  "Heathrow Airport, Terminal 5, Hounslow, Middlesex. TW6 2GA",

  "Heathrow Airport, Terminal 4, Hounslow, Middlesex. TW6 3XA",

  "Heathrow Airport, Terminal 3, Hounslow, Middlesex. TW6 1QG",

  "Heathrow Airport, Terminal 2, Hounslow, Middlesex. TW6 1EW",

  "Gatwick Airport, South Terminal, West Sussex. RH6 ONP",

  "Gatwick Airport, North Terminal, West Sussex. RH6 ONP",

  "Farnborough Airport, Farnborough, London. GU14 6XA",

  "Birmingham Airport, Birmingham. B26 3QL",

  "London Biggin Hill Airport Ltd",
];

export { details, features, paymentLogos, airports };
